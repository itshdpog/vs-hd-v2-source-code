...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				var smphdfront:BGSprite = new BGSprite('smphdfront', -200, 0);
				smphdfront.setGraphicSize(Std.int(smphdfront.width * 1.1));
				smphdfront.updateHitbox();
				add(smphdfront);

				case 'Kickbg': //smphds stage
				var kickhouse:BGSprite = new BGSprite('kickhouse', -170, -70);
				add(kickhouse);

				case 'roblox': //roblox stage
				var robloz:BGSprite = new BGSprite('Robloz', -170, -70);
				add(robloz);

				case 'season3': //season 3 stage
				var s3back:BGSprite = new BGSprite('nightsky', -170, -70, 0.9, 0.9);
				add(s3back);

				var s3front:BGSprite = new BGSprite('bruhh', -200, 10);
				s3front.setGraphicSize(Std.int(s3front.width * 1.2));
				s3front.updateHitbox();
				add(s3front);
				...
				var smphdfront1:BGSprite = new BGSprite('smphdfront', -200, 0);
				...
				case 'playstore': //smphddaystage
				var playstore:BGSprite = new BGSprite('Playstore', -350, -70);
				playstore.setGraphicSize(Std.int(playstore.width * 1.1));
				playstore.updateHitbox();
				add(playstore);
		...
		switch(Paths.formatToSongPath(SONG.song))
		{
			...
			case 'Ares':
				SongCreator = 'Sixteenj';
				ChartCreator = 'Techsec';
			case 'torment-old':
				SongCreator = 'KickaFly';
				ChartCreator = 'RhyThmyX';
			case 'ares-old':
				SongCreator = 'KickaFly';
				ChartCreator = 'Techsec';
			case 'Mediocre':
				SongCreator = 'Sixteenj & Just Maddie';
				ChartCreator = 'Just Maddie';
		}
		...
		timeTxt = new FlxText(0, 15, 400, "", 32);
		...
		timeBar.createFilledBar(0xFF3A3A3A, 0xFFFFFFFF);
		...
		wipTxt = new FlxText(150, 20, FlxG.width, "WIP BUILD", 670);
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 150, FlxG.width - 800, "I AM BAD AT THE GAME", 32);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.F1 || FlxG.keys.justPressed.F2)
			{
		//		FlxG.switchState(new HackedState());
		PlayState.SONG = Song.loadFromJson("torment-old", "torment-old"); //Torment Loader
		FlxG.switchState(new PlayState());
			}
...
/*			if (FlxG.keys.justPressed.F2)
				...
				if (FlxG.keys.justPressed.F3)*/
					...
					if (FlxG.keys.justPressed.F4)
						{
							if (curSong.toLowerCase() == 'Disc' || curSong.toLowerCase() == 'ares')
							{
								FlxG.switchState(new HackedState());
							}
						 }
											...
											if (FlxG.keys.justPressed.F10)
												...
												if (FlxG.keys.justPressed.F12)
													...
													if (FlxG.keys.justPressed.ESCAPE)
														{
															PlayState.SONG = Song.loadFromJson("torment", "torment"); //Song Loading Thing
															FlxG.switchState(new PlayState());
														}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					FlxG.sound.playMusic(Paths.music('freakyHD'));
