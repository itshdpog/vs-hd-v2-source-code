...
class HackedState extends MusicBeatState
{
	...
	override function create()
	{
		...
		tSpr.antialiasing = true;
			...
			txt = new FlxText(0, 0, FlxG.width, "HEY! Kickafly has hacked your game!\nThats very unfortunate...\nThe only way to go back to playing is by subscribing to Kickafly.\n(Pressing B is useless until subscribing)", 32);
	...
	{	
		if (controls.ACCEPT)
		{
			FlxG.openURL("https://www.youtube.com/c/KickaFly");
			subscribed = true;
		}
		if (controls.BACK)
		{
			...
			{
				tSpr.visible = true;
				txt.text = "Please subscribe to Kickafly first.";
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					txt.text = "HEY! Kickafly has hacked your game!\nThats very unfortunate...\nThe only way to go back to playing is by subscribing to Kickafly.\n(Pressing B is useless until subscribing)";
					tSpr.visible = false;
				});
			}
		}
